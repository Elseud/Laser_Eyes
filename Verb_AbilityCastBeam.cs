...
namespace UFO.CastBeam
{
    ...
    public class Verb_AbilityCastBeam : Verb_CastAbility
    {
        ...
        public override float? AimAngleOverride => state != VerbState.Bursting
            ? new float?()
            : (InterpolatedPosition - caster.DrawPos).AngleFlat();
        ...
        protected override bool TryCastShot()
        {
            if (currentTarget.HasThing && currentTarget.Thing.Map != caster.Map)
                return false;
            var shootLineFromTo =
                TryFindShootLineFromTo(caster.Position, currentTarget, out ShootLine _);
        ...
        private void ApplyDamage(Thing thing)
        {
            ...
            DamageInfo dinfo = new DamageInfo(verbProps.beamDamageDef,
                ...
                angleFlat, caster, weapon: caster.def,
            ...
            else
            {
                if (!Rand.Chance(verbProps.beamChanceToStartFire))
                    return;
                FireUtility.TryStartFireIn(intVec3_1, map, verbProps.beamFireSizeRange.RandomInRange);
            }
        }
